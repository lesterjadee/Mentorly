    ...
    <main className="min-h-screen bg-[#080C14] text-white overflow-hidden">
      ...
      <nav className="flex items-center justify-between px-6 md:px-8 py-5 border-b border-white/5 animate-fade-in">
        ...
        <div className="flex items-center gap-3">
          <Link href="/login" className="text-sm text-white/60 hover:text-white transition-colors duration-200 px-4 py-2">
            Sign in
          </Link>
          <Link href="/register" className="text-sm bg-[#26619C] hover:bg-[#1e4f82] active:scale-95 transition-all duration-200 px-4 py-2 rounded-lg font-medium flex items-center gap-1.5">
            Get started
            <ArrowRight size={13} />
          </Link>
        </div>
      ...
      <section className="relative px-6 md:px-8 pt-20 md:pt-28 pb-20 max-w-6xl mx-auto">
        ...
        <div className="relative text-center">
          ...
          <p className="text-base md:text-lg text-white/50 max-w-xl mx-auto mb-10 leading-relaxed animate-fade-in-up delay-75">
          ...
          <div className="flex items-center justify-center gap-4 flex-wrap animate-fade-in-up delay-150">
            <Link href="/register" className="bg-[#26619C] hover:bg-[#1e4f82] active:scale-95 transition-all duration-200 px-6 py-3 rounded-xl font-medium text-sm flex items-center gap-2">
              Find a tutor
              <ArrowRight size={14} />
            </Link>
            <Link href="/register" className="bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 active:scale-95 transition-all duration-200 px-6 py-3 rounded-xl font-medium text-sm text-white/80">
              Offer your skills
            </Link>
          </div>
        </div>
        ...
        {/* stats */}
        <div className="grid grid-cols-3 gap-3 md:gap-4 mt-16 md:mt-20 max-w-2xl mx-auto animate-fade-in-up delay-225">
          {[
            { value: '2,400+', label: 'Active tutors' },
            { value: '18 subjects', label: 'Categories' },
            { value: '4.9 / 5', label: 'Avg. rating' },
          ].map((stat) => (
            <div key={stat.label} className="text-center glass rounded-2xl py-5 md:py-6 px-4 hover:border-[#26619C]/20 transition-colors duration-300">
      ...
      <section className="px-6 md:px-8 py-16 max-w-6xl mx-auto">
        <div className="grid md:grid-cols-3 gap-4">
          {[
            { icon: Shield, title: 'Verified student profiles', desc: 'Every tutor is a real enrolled student with a trust score built from reviews and session history.', color: 'text-[#4a8fd4]', bg: 'bg-[#26619C]/10', border: 'border-[#26619C]/20', hover: 'hover:border-[#26619C]/40' },
            { icon: Zap, title: 'Instant booking', desc: 'Browse, filter, and book a session in under 2 minutes. Online or in-person — your choice.', color: 'text-teal-400', bg: 'bg-teal-500/10', border: 'border-teal-500/20', hover: 'hover:border-teal-500/40' },
            { icon: Star, title: 'Fair pricing', desc: 'Students set their own rates. Get quality help without paying agency fees or premium prices.', color: 'text-yellow-400', bg: 'bg-yellow-500/10', border: 'border-yellow-500/20', hover: 'hover:border-yellow-500/40' },
          ].map((f, i) => (
            <div
              key={f.title}
      ...
      <section className="px-6 md:px-8 py-8 max-w-6xl mx-auto mb-8">
        <div className="relative glass rounded-2xl p-8 md:p-10 overflow-hidden text-center">
          ...
          <div className="relative">
            ...
            <Link href="/register" className="inline-flex items-center gap-2 bg-[#26619C] hover:bg-[#1e4f82] active:scale-95 transition-all duration-200 px-6 py-3 rounded-xl font-medium text-sm">
              Create free account
              <ArrowRight size={14} />
            </Link>
          </div>
      ...
      <footer className="border-t border-white/5 px-6 md:px-8 py-6 max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <p className="text-xs text-white/20">© 2025 Mentorly</p>
          </div>
